...
# Example Godot HTTP client showing how to POST a simulation request and
# receive results. Attach to a Node and add an `HTTPRequest` child named
# `HTTPRequest` (or change the node path below). This client implements basic
# retry logic and clearer error handling for network/server failures.
...
# Optional API key loading (looks for user://api_key.cfg written by the
# `api_key_manager.gd` helper). If present, the key is sent as the
...
func _ready() -> void:
    # Ensure there's an HTTPRequest node (create if missing)
    http = get_node_or_null("HTTPRequest")
    if http == null:
        http = HTTPRequest.new()
        add_child(http)
    # Connect signal (idempotent)
...
func _on_HTTPRequest_request_completed(result, response_code, headers, body):
    # result is an OK code from HTTPRequest; response_code is the HTTP status
    if response_code >= 200 and response_code < 300:
        var s = body.get_string_from_utf8()
        var parsed = JSON.parse(s)
        if parsed.error == OK:
            var data = parsed.result
            # data contains times, energies, norms, states_real, states_imag, logs
        ...
        # treat 5xx as retryable, 4xx as client error
